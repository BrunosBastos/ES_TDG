...
def test_fill_docx_file():
    ...
    test_path = "test/templates/word/Templates/1_template_simpleWord/"
    ...
    s3.put_object(Bucket='tdg-s3-bucket', Key=path, Body=open(test_path + "input_simpleWord-Template.docx", "rb"))

    response = test_app.post(
        "/api/3/fill", files={"upload_file": open(test_path + "input_simpleWord-Data.json", "rb")},
        data={"retrieval_filename": path, "output_filename": output_filename}
    )
